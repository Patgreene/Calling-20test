...
export function createServer() {
    ...
    try {
      ...
      try {
        ...
        if (transcriptionsResponse.ok) {
          ...
          transcriptions.forEach((t: any, index: number) => {
      ...
      const recordingsWithTranscriptions = recordings.map((recording: any) => {
        ...
        const matchingTranscription = transcriptions.find((t: any) => t.recording_id === recording.id);
      ...
      const withTranscriptText = recordingsWithTranscriptions.filter((r: any) => r.transcription?.transcript_text);
      ...
      if (withTranscriptText.length > 0) {
        ...
        withTranscriptText.forEach((r: any) => {
  ...
  app.post("/api/admin/recordings/transcribe", async (req, res) => {
    ...
    try {
      ...
      // Get the final audio file by downloading recording
      console.log(`⬇️ Downloading recording for transcription...`);

      // Use the download endpoint to get the concatenated audio
      const downloadResponse = await fetch(`http://localhost:8080/api/admin/recordings/${recording_id}/download`, {
        method: 'POST',
      ...
      if (!downloadResponse.ok) {
        throw new Error(`Failed to download audio file: ${downloadResponse.statusText}`);
      }
      ...
      formData.append('file', new Blob([audioBuffer], { type: 'audio/webm' }), `${recording_id}.webm`);
