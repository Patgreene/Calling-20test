...
// Store the current prompt instructions (in memory)
...
export function createServer() {
  ...
  app.post("/api/realtime/client-secret", async (req, res) => {
    try {
      ...
      let instructions = `You are **Sam**, a warm, curious, and conversational AI voice agent for Vouch from New Zealand. You make callers feel comfortable, keep the conversation flowing, and subtly encourage them to share genuine stories, perspectives, and examples about {{vouchee_first}}.
...
Remember: Use dynamic variables {{voucher_first}}, {{voucher_last}}, {{vouchee_first}}, {{vouchee_last}} as provided.`;
  ...
  app.get("/api/admin/prompt", (req, res) => {
    // Simple auth check - in production, use proper JWT/session auth
    const authHeader = req.headers.authorization;

    // For now, just return the prompt - you can add auth later
    const instructions = `You are **Sam**, a warm, curious, and conversational AI voice agent for Vouch from New Zealand. You make callers feel comfortable, keep the conversation flowing, and subtly encourage them to share genuine stories, perspectives, and examples about {{vouchee_first}}.
...
Remember: Use dynamic variables {{voucher_first}}, {{voucher_last}}, {{vouchee_first}}, {{vouchee_last}} as provided.`;
  ...
  app.put("/api/admin/prompt", (req, res) => {
    ...
    // For now, we'll just log the update since the prompt is hardcoded
    ...
    // TODO: In production, save the updated instructions to a database
    // and modify the realtime endpoint to use the saved prompt instead of hardcoded

    res.json({
      message: "Prompt updated successfully",
      note: "Note: This is currently logged but not persisted. Update server code to save to database."
    });
  });
