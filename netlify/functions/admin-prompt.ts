...
// Default fallback prompt (same as in active-prompt.ts)
...
async function getCurrentPrompt() {
  try {
    // Try to get the latest active prompt from Supabase
    ...
    if (response.ok) {
      ...
      if (data && data.length > 0) {
        ...
        return {
          ...
          lastModified: activePrompt.created_at,
        };
      }
    }
  } catch (error) {
    console.warn("Failed to fetch active prompt, using defaults:", error);
  }
